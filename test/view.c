...
#define BROKEN_TIOCGWINSZ 1	/* ...at least there's no 'resizeterm()' */
#endif
...
#if defined(SIGWINCH) && defined(TIOCGWINSZ) && !defined(BROKEN_TIOCGWINSZ)
#define CAN_RESIZE 1
#else
#define CAN_RESIZE 0
#endif
...
static void show_all(void)
{
	...
	scrollok(stdscr, FALSE); /* prevent screen from moving */
        for (i = 1; i < LINES; i++) {
            move(i, 0);
            if ((s = lptr[i-1]) != 0 && strlen(s) > shift)
