...
--  |
   ...
   pragma Import (C, Normalize_Field_Options, "_nc_normalize_field_opts");
...
--  |
   ...
   pragma Import (C, Normalize_Form_Options, "_nc_normalize_form_opts");
...
------------------------------------------------------------------------------
   ...
   function Search_Type (T : Tag) return C_Field_Type
   is
      P : Tag_Pair_Access := External_Pair_Htable.Get (T);
   begin
      if P /= null then
         return P.Cft;
      else
   ...
   procedure Register_Type (T   : in Tag;
                            Cft : in C_Field_Type)
   is
...
----------------------------------------------------------------------------
   ...
   is
      ...
      Res : Eti_Error;
      C   : constant C_Field_Type := Search_Type (Fld_Type'Tag);
   begin
      if C = Null_Field_Type then
         raise Form_Exception;
      else
   ...
   function Native_Type (Ftype : Ada_Defined_Field_Type)
                         return C_Field_Type
   is
      C : constant C_Field_Type := Search_Type (Ftype'Tag);
   begin
      if C = Null_Field_Type then
         raise Form_Exception;
      else
   ...
   begin
      Res := Set_Fld_Type (Arg1 => C_Int (Fld_Type.Precision),
                           Arg2 => C_Int (Fld_Type.Lower_Limit),
                           Arg3 => C_Int (Fld_Type.Upper_Limit));
      if Res /= E_Ok then
         Eti_Exception (Res);
      else
