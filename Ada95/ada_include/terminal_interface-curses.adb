...
------------------------------------------------------------------------------
   subtype Column_Range is Integer range
     Integer (Column_Position'First) .. Integer (Column_Position'Last);
   subtype Line_Range is Integer range
     Integer (Line_Position'First) .. Integer (Line_Position'Last);

   function "+" (Left : Column_Position; Right : Integer)
     return Column_Position
   is
      Res : constant Integer := Integer (Left) + Right;
   begin
      if Res not in Column_Range then
         raise Constraint_Error;
      end if;
      return Column_Position (Res);
   end "+";

   function "+" (Left : Integer; Right : Column_Position)
     return Column_Position
   is
      Res : constant Integer := Left + Integer (Right);
   begin
      if Res not in Column_Range then
         raise Constraint_Error;
      end if;
   ...
   function "+" (Left : Line_Position; Right : Integer)
     return Line_Position
   is
      Res : constant Integer := Integer (Left) + Right;
   begin
      if Res not in Line_Range then
         raise Constraint_Error;
      end if;
      return Line_Position (Res);
   end "+";

   function "+" (Left : Integer; Right : Line_Position)
     return Line_Position
   is
      Res : constant Integer := Left + Integer (Right);
   begin
