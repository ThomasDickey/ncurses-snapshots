dnl $Id: aclocal.m4,v 1.21 1996/06/02 00:07:05 tom Exp $
...
NC_EOF
		for i in `cat $srcdir/$nc_dir/headers`
		do
			echo "	@ ../headers.sh \$(INSTALL_DATA) \$(includedir) \$(srcdir) $i" >>$nc_dir/Makefile
		done
	fi
done
  ...
  for nc_dir in $MANPATH; do
    ...
    do
	  ...
	  *)    nc_form=cat;;
	  ...
	  # (I'm assuming Debian installs data.1m)
	  case "$nc_name" in
	  *.1m)	nc_rename=`cd $srcdir;pwd`/man/man_db.renames;;
	  *)	nc_rename="";;
	  esac
	  break
       fi
